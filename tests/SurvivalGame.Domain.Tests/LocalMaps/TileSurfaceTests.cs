...
{
    [Fact]
    public void SurfaceIdTrimsValueAndFormatsAsRawId()
    {
        var id = new SurfaceId("  asphalt  ");

        Assert.Equal("asphalt", id.Value);
        Assert.Equal("asphalt", id.ToString());
    }

    [Theory]
    [InlineData("")]
    [InlineData("   ")]
    public void SurfaceIdRejectsEmptyValues(string value)
    {
        Assert.Throws<ArgumentException>(() => new SurfaceId(value));
    }

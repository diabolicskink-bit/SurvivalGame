...
- `src/SurvivalGame.Domain/Actions/` contains action requests, action resolution, world time state, and the current local gameplay state.
...
- `LocalMapView/MapEntityLayer.cs` renders world objects, NPCs, and the player marker in one Y-sorted pass. Object and NPC sprites may use optional `spriteRender` metadata to draw beyond their owning tile without changing collision, hover, targeting, or placement state.

...
public partial class GroundItemLayer : Node2D
{
    ...
    public override void _Draw()
    {
        ...
        {
            if (TryMapToViewport(placedItem.Position, out var viewportPosition))
            {
                DrawItemMarker(placedItem.Stack, viewportPosition);
            }
        }
    ...
    private void DrawItemMarker(GroundItemStack stack, GridPosition viewportPosition)
    {
        var center = CellToBoardPosition(viewportPosition);
        if (TryGetItemSprite(stack.ItemId, out var sprite))
        {
        ...
        var color = GetItemColor(stack.ItemId);
